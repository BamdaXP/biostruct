...
def get_msa(entry:Entry, chain_id:str, db:str, ncpu=4, redo=False):
	...
	sto_filepath = entry.get_path(f'msa/{db}/{chain_id}.sto')
	...
	if not (os.path.isfile(msa_filepath) and os.path.getsize(msa_filepath)) or redo:
		#seach if not exist
		if not(os.path.isfile(sto_filepath) and os.path.getsize(sto_filepath)) or redo:
			...
			#run
			os.system(cmd)
		if not (os.path.isfile(mask_filepath) and os.path.getsize(mask_filepath)) or redo:
