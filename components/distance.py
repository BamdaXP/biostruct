...
CONTACT_FILENAME_PATTERN = r"(\S+)_(\S+)\(\d+\)\.dist"

def get_distance(entry:Entry, pair:tuple, redo=False) -> np.ndarray:
	...
	distance_filepath = entry.get_path(
		f'contacts/{chain1_id}_{chain2_id}.dist')
	map_data = None
	#load if already exist
	if os.path.isfile(distance_filepath) and not redo:
		map_data = np.loadtxt(distance_filepath)
	else:
		#calculate and save if not exist
		print(f"Calculating distance {entry}:{pair} ...",file=sys.stdout)
		...
		for animo1 in chain1_model.child_list:
			...
			for animo2 in chain2_model.child_list:
				...
				d = np.min(distmap)
				map_data[map_index1, map_index2] = d
		...
		#save mapdata
		np.savetxt(distance_filepath, map_data)
